...
import com.squareup.workflow.WorkflowHandle.FinishedWorkflow
import com.squareup.workflow.WorkflowHandle.RunningWorkflow
...
class WorkflowPoolTest {
  ...
  fun handle(
    state: String = "",
    name: String = ""
  ): RunningWorkflow<String, String, String> = RunningWorkflow(
      myReactor.workflowType.makeWorkflowId(name), state
  )
  ...
  @Test fun `waits for state after current`() {
    ...
    val input = pool.input(handle.id)
    input.sendEvent(NEW)
    input.sendEvent(NEW)
    input.sendEvent(NEW)
    assertFalse(nestedStateSub.isCompleted)
