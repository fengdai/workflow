...
class Rx2WorkflowPoolIntegrationTest {
  ...
  private fun handle(
    name: String = "",
    state: String = ""
  ) = RunningWorkflow(myReactor.workflowType.makeWorkflowId(name), state)
  ...
  @Test fun `waits for state after current`() {
    ...
    nestedStateSub.assertValue(RunningWorkflow(handle.id, "fnord"))
...
/**
 * Helper for calling and subscribing to [com.squareup.workflow.rx2.onWorkflowUpdate] for tests.
 */
